...
const mappings = {
    ...
    userName: 'username',
    name: {
        __root: 'name,given_name,family_name',
        formatted: 'name',
...
function toAuth0Fields(scimAttributes) {
    ...
    if (scimAttributes.indexOf('id') === -1) scimAttributes.push('id');

    // convert scim attributes to auth0 fields
    let auth0Fields = (scimAttributes || []).reduce(function (result, scimAttribute) {
        scimAttribute = scimAttribute.replace("urn:auth0:2.0:User", "auth0User");
        //const auth0param = jp.query(mappings, scimAttribute);
        const auth0param = [];
